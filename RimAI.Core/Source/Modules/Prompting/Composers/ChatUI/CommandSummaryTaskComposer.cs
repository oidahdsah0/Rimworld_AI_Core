...
{
    // 在 ChatUI 作用域下，仅当 IsCommand=true 时注入“当下任务”系统提示
    internal sealed class CommandSummaryTaskComposer : IPromptComposer
    {
        public PromptScope Scope => PromptScope.ChatUI;
    public int Order => 9998; // 确保位于系统行的最末尾（除仅提供User前缀的作曲器外）
        public string Id => "chatui_command_summary_task";
