...
- 范围（本阶段交付）
  ...
  - 右列四行布局：标题、聊天主体（斑马栅格/圆弧边框）、指示灯（Data/Busy/Fin）、输入行（多行富文本 + 三按钮 + 快捷键）。
...
- 右列四行（瀑布布局，高度权重建议 1:7:1:3，可按 UI 观感微调）
  1) 标题行：头像 + 名称 + 职务（未设置→“无职务”）。
  2) 聊天主体：
     - Chatbot 风格、圆弧边框；滚动区域自下向上或自上向下均可，但需保持新增时可见。
     - 玩家与 AI 发言统一靠左；行内包含“名称 + 时间戳”。
     - 斑马栅格：玩家/AI 使用不同淡色底；行间距与内边距统一。
  3) 指示灯行：
...
- 布局与交互
  ...
  - 标题行展示头像/名称/职务（未设置→“无职务”）。
  - 聊天主体：玩家/AI 左对齐、名称+时间戳、斑马栅格、滚动稳定。
